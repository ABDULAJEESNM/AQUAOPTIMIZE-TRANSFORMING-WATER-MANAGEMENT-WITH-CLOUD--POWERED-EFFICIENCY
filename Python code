...
def insert_data(data):
    try:
        ...
        if len(values) == 3:
            ...
            sql_insert_query = "INSERT INTO water_management (temperature,humidity,waterlevel) VALUES (%s, %s, %s)"

            # Execute the SQL query with data parameters
            cursor.execute(sql_insert_query, (temperature,humidity,waterlevel,))

            # Commit the transaction
            connection.commit()

            print(f"Data inserted: temperature={temperature},  humidity={humidity},waterlevel={waterlevel}, ")
